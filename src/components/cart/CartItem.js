export const CartItem = ({ id, title, price, image, quantity, selected }) => {
  return /* html */ `
  <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${id}">
          ...
          <label class="flex items-center mr-3">
            ...
            focus:ring-blue-500" data-product-id="${id}" ${selected ? "checked" : ""}>
          </label>
          <!-- 상품 이미지 -->
          <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden mr-3 flex-shrink-0">
            <img src="${image}" alt="${title}" class="w-full h-full object-cover cursor-pointer cart-item-image" data-product-id="${id}">
          </div>
          <!-- 상품 정보 -->
