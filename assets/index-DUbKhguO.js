(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e){if(t.type!==`childList`)continue;for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();const e=e=>{if(!e||e===`?`)return{};let t=new URLSearchParams(e),n=Object.fromEntries(t.entries());return n},t=e=>{if(!e||Object.keys(e).length===0)return``;let t=new URLSearchParams(e);return t.toString()},n=e=>{if(e===`*`)return/./;let t=e.replace(/\//g,`\\/`),n=t.replace(/:(\w+)/g,`(?<$1>[^/]+)`);return RegExp(`^${n}$`)},r=(e,t)=>{let r=n(e),i=r.exec(t);if(!i)return{matched:!1,params:{}};let a=i.groups||{};return{matched:!0,params:a}},i=(e,t)=>{for(let n of e){let e=r(n.path,t);if(e.matched)return{route:n,params:e.params}}return null},a=async t=>{let n=location.pathname,r=`/front_7th_chapter2-1/`;r!==`/`&&n.startsWith(r)&&(n=n.slice(r.length)||`/`);let a=location.search,o=i(t,n);if(!o){console.error(`[오류] 매칭되는 라우트 없음:`,n);return}let{route:s,params:c}=o,l=e(a),u=await s.component({params:c,query:l}),d=document.querySelector(`#root`);d.innerHTML=u},o=(e,n,r={})=>{let i=t(r),o=i?`${n}?${i}`:n;history.pushState({},``,o),a(e)},s=e=>{document.addEventListener(`click`,t=>{let n=t.target.closest(`[data-link]`);if(n){t.preventDefault();let r=n.getAttribute(`href`);o(e,r);return}let r=t.target.closest(`.product-card`);if(r){if(t.target.closest(`.add-to-cart-btn`))return;let n=r.dataset.productId;o(e,`/product/${n}`);return}})},c=e=>{window.addEventListener(`popstate`,()=>{a(e)})},l=e=>(s(e),c(e),a(e),{push:(t,n={})=>o(e,t,n)}),u=()=>`
  ...
  `,_=({title:e,image:t,lprice:n,mallName:r,productId:i})=>`
    <div
  ...
  `;async function x(e={}){let{limit:t=20,search:n=``,category1:r=``,category2:i=``,sort:a=`price_asc`}=e,o=e.current??e.page??1,s=new URLSearchParams({page:o.toString(),limit:t.toString(),...n&&{search:n},...r&&{category1:r},...i&&{category2:i},sort:a}),c=await fetch(`/api/products?${s}`);return await c.json()}async function S(e){let t=await fetch(`/api/products/${e}`);return await t.json()}const C=async({query:e={}})=>{let t={search:e.search||``,category1:e.category1||``,category2:e.category2||``,sort:e.sort||`price_asc`,limit:parseInt(e.limit)||20},n=await x(t);return f({children:`
      ${g({filters:t,pagination:n.pagination})}
      ${b({loading:!1,products:n.products})}
    `})},w=({category1:e,category2:t})=>`
    <nav class="mb-4">
  ...
  `,T=e=>{let t=Math.floor(e),n=5-t,r=`M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z`,i=`<svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="${r}"></path></svg>`,a=`<svg class="w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20"><path d="${r}"></path></svg>`;return i.repeat(t)+a.repeat(n)},E=e=>`
  ...
  `,k=async({params:e={}})=>{let t=e.id,n=await S(t);return f({children:`
      <main class="max-w-md mx-auto px-4 py-4">
        ${w({category1:n.category1,category2:n.category2})}
        ${E(n)}
        ${D({productId:n.productId,stock:n.stock})}
        <div class="mb-6">
          <a
            href="/"
            data-link
  ...
  `}),j=[{path:`/`,component:C},{path:`/product/:id`,component:k},{path:`*`,component:A}];async function M(){let e=l(j);window.router=e}M();